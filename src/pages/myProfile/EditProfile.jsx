...
import { Card, Avatar, Button, Form, Input, Tabs, Typography, Space, notification, Upload, Row, Col, Divider, Radio } from "antd";
...
export default function EditProfile({ user, setIsEditProfile }) {
  ...
  useEffect(() => {
    if (user) {
      form.setFieldsValue({
        ...user,
        have_child: user.have_child ? "Yes" : "No", // Update this field with the correct value
      });
    }
  }, [user]);
  ...
  const onFinish = async (values) => {
    values = {
      ...values,
      have_child:values.have_child === 'Yes'
    }
    try {
      await updateUser(
        {
  ...
  const tabs = [
    ...
    { key: "professionalInfo", tab: "Professional", icon: <BookOutlined /> },
            ...
            <Form layout="vertical">
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please upload a profile document!",
                  },
                ]}
              >
                <Upload
                  ...
                  beforeUpload={(file) => {
                    ...
                    if (!isAllowedFile) {
                      alert(
                        "You can only upload JPG, PNG, PDF, DOC, or DOCX files!"
                      );
